...
def get_imdb_ids():
    ...
    for movie in req:
        ...
        list_api_url_r = 'movies/{0}/ratings'.format(imdb)
        req = get_oauth_request(list_api_url_r)
        if req["votes"] >= config.VOTES and movie["movie"]["year"] >= config.YEAR and req["rating"] >= config.RATING:
            movies.add(imdb)
    return movies
...
def main():
    ...
    already_added = localdb.get('already_added_watched', [])
    post_data = []
    movies_in_watched_list = get_imdb_ids()

    for imdb in already_added[:]:
        if imdb in movies_in_watched_list:
            continue
